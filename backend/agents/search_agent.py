...
#                     if not results:
#                         results = await self._alternative_search(query, max_results)
...
#             return await self._alternative_search(query, max_results)
    
#     async def _alternative_search(self, query: str, max_results: int) -> List[Dict[str, Any]]:
...
#                 # If Wikipedia fails, generate synthetic results
...
#         """Generate synthetic search results using Gemini"""
#         try:
#             prompt = f"""
...
#                     'content': f'This search result contains information about {query}. The content includes relevant details and insights related to the query.',
#                     'type': 'synthetic'
#                 }]
...
#                 summary_prompt = f"""
#                 Based on the following search results about "{query}", provide a comprehensive summary:
                
#                 {content_text}
                
#                 Create a well-structured summary that:
#                 1. Highlights the key findings
#                 2. Identifies important trends or patterns
